...
`,
  ...
  SESSION_WINDDOWN_HOOK: `#!/usr/bin/env node
 ...
 * 
 * Trigger phrases:
 * - "goodbye", "wind down", "end session", "wrap up", "finish session"
 */
...
export async function installCriticalComponents(projectPath, variables = {}) {
  ...
  // Install session-winddown.js hook (NEW: Added in v1.2.0)
  const sessionWinddownPath = path.join(hooksDir, 'session-winddown.js');
  if (!fs.existsSync(sessionWinddownPath)) {
    await fs.promises.writeFile(sessionWinddownPath, TEMPLATES.SESSION_WINDDOWN_HOOK, 'utf8');
    fs.chmodSync(sessionWinddownPath, 0o755); // Make executable
    installed.push('.claude/hooks/session-winddown.js');
  }
...
export async function repairDiet103Infrastructure(projectPath, options = {}) {
  ...
  // Install Session Wind-Down System (NEW: Added in v1.2.0)
  result.installed.sessionWinddown = [];
  
  try {
    const orchestratorPath = path.dirname(path.dirname(__dirname)); // Up from lib/utils/
    ...
    // Copy session-cleanup agent if it doesn't exist
    const sessionCleanupSrc = path.join(orchestratorPath, '.claude', 'agents', 'session-cleanup.md');
    const sessionCleanupDest = path.join(agentsDir, 'session-cleanup.md');
    
    if (fs.existsSync(sessionCleanupSrc) && !fs.existsSync(sessionCleanupDest)) {
      fs.copyFileSync(sessionCleanupSrc, sessionCleanupDest);
      result.installed.sessionWinddown.push('.claude/agents/session-cleanup.md');
    }
    
    // Copy wind-down command if it doesn't exist
    const windDownCmdSrc = path.join(orchestratorPath, '.claude', 'commands', 'wind-down.md');
    const windDownCmdDest = path.join(commandsDir, 'wind-down.md');
    
    if (fs.existsSync(windDownCmdSrc) && !fs.existsSync(windDownCmdDest)) {
      fs.copyFileSync(windDownCmdSrc, windDownCmdDest);
      result.installed.sessionWinddown.push('.claude/commands/wind-down.md');
    }
  } catch (error) {
    // Session wind-down installation is non-critical, log but don't fail
    console.warn(`  ! Session wind-down installation warning: ${error.message}`);
  }
