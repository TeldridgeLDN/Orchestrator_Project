...
export async function healthCommand(projectPath, options = {}) {
  ...
  try {
    ...
    const projectRoot = detectProjectRoot(resolvedPath);
...
function displayHealthReport(health, verbose = false, recommendationResult = null) {
  ...
  displayComponentScore('Structure', health.components.structure, 40, verbose);
  displayComponentScore('Hooks', health.components.hooks, 30, verbose);
