...
for i in {1..12}; do
  CLAUDE_HOME=~/.claude-test $CLAUDE_CLI project create "project-$i" --template base
...
for i in {1..12}; do
  start_time=$(date +%s.%N)
  CLAUDE_HOME=~/.claude-test $CLAUDE_CLI project switch "project-$i"
  end_time=$(date +%s.%N)
...
if command -v "CLAUDE_HOME=~/.claude-test $CLAUDE_CLI nl" &> /dev/null; then
  echo "Testing natural language commands..."
  CLAUDE_HOME=~/.claude-test $CLAUDE_CLI nl "switch to project 5"
  CURRENT=$(CLAUDE_HOME=~/.claude-test $CLAUDE_CLI project current)
